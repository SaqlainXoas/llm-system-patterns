...
|---|---|---|
| `Pre-filter` | Remove clearly invalid proposals | Cheapest place to enforce hard constraints |
...
- document-type or submission-status checks
...
For bulk proposal processing, the same shape works well:

```text
extract and parse proposal
-> exact capability or acronym pre-check
-> embedding similarity against the brief
-> optional rerank or score normalization
-> final LLM validation on the best few proposals
```

That is very different from a naive LLM-first design, and it is also different from "store everything in a vector DB first and figure it out later." If the proposals are being scored in one pipeline run, you can often parse, embed, score, and clean up in-memory without a vector DB at all.

## Implementation blueprint
For a proposal-to-brief matcher or document scorer, a beginner-friendly first version is easier to understand when it runs top to bottom:
...
`metadata / regex filters` usually means region checks, exact capability names, explicit compliance tags, document type checks, or numeric thresholds that should not be left to the LLM.

`hybrid retrieval` usually means keyword top-k plus embedding top-k merged into one proposal set. This is especially useful when the system must respect abbreviations like `SOC 2`, `SSO`, `ISO 27001`, or `C++` but still recover paraphrases and related meaning.
...
[![Example](https://img.shields.io/badge/Example-Proposal%20Scoring-16a34a)](../examples/proposal-to-brief-matching-basic/README.md)
